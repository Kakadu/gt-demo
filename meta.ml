...
class ['a, 'b] show_pv self fa fb =
  object
    inherit ['a, unit, string, 'b, unit, string, unit, string] class_pv
    method c_A () a = "`A (" ^ fa a ^ ")"
    method c_B () b = "`B (" ^ fb b ^ ")"
  end

class ['a, 'a1, 'b, 'b1] map_pv self (fa : 'a -> 'a1) (fb : 'b -> 'b1) =
  object
    inherit ['a, unit, 'a1, 'b, unit, 'b1, unit
            , ('a1, 'b1) pv] class_pv
    method c_A () a = `A (fa a)
    method c_B () b = `B (fb b)
  end
...
class virtual ['a, 'ia, 'sa, 'b, 'ib, 'sb, 'inh, 'syn] class_pv_ext =
  object
    inherit ['a, 'ia, 'sa, 'b, 'ib, 'sb, 'inh, 'syn] class_pv
    method virtual c_C : 'inh -> 'a -> 'syn
  end
class ['a, 'b] show_pv_ext self fa fb =
  object
    inherit ['a, unit, string, 'b, unit, string, unit, string] class_pv_ext
    inherit ['a, 'b] show_pv self fa fb
    method c_C () a = "`A (" ^ fa a ^ ")"
  end

class ['a, 'a1, 'b, 'b1] map_pv_ext self (fa : 'a -> 'a1) (fb : 'b -> 'b1) =
  object
    inherit ['a, unit, 'a1, 'b, unit, 'b1, unit, ('a1, 'b1) #pv_ext] class_pv_ext
    inherit ['a, 'a1, 'b, 'b1 ] map_pv self fa fb
    method c_C () a = `C (fa a)
  end
