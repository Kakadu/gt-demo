...
type a_trf  = { a_trf: 'a 'sa 's. ('a -> 'sa) -> 'a a -> 's }
type b_trf  = { b_trf:        's . b -> 's }
type c_trf  = { c_trf:        's . c -> 's }
type d_trf  = { d_trf:        's . d -> 's }

type a_trf_self  = { a_trf_self: 'a     's . 'a a -> 's }
type b_trf_self  = { b_trf_self:        's . b -> 's }
type c_trf_self  = { c_trf_self:        's . c -> 's }
type d_trf_self  = { d_trf_self:        's . d -> 's }
...
class ['l, 'self] show_a_stub for_a for_b for_c for_d ~fself fl = object
  ...
  method c_C ()    = "C"
  method c_E () a  = sprintf "E (%a)" (fun () -> fself.a_trf_self) a
...
class ['self] show_c_stub for_a for_b for_c for_d ~fself = object
  inherit [string, 'self] show_a_stub for_a for_b for_c for_d
      ~fself:({a_trf_self = for_a.a_trf id}) id
end
class ['self] show_d_stub for_a for_b for_c for_d ~fself = object
  inherit [c] show_list (fun () -> fself)
      (fun () x -> for_a.a_trf (sprintf "%S") x)
end
(* Type of first arguments of generated classes *)

type typ_for_a =
  { a_func : 'a 'sa 's 'selfa .
               a_trf -> b_trf -> c_trf -> d_trf -> fself:a_trf ->
      ('a -> 'sa) ->
      ('a, 'selfa) show_a_stub
  }
type typ_for_b =
  { b_func : 'self 's .
                a_trf -> b_trf -> c_trf -> d_trf -> fself:(b -> 's) ->
      'self show_b_stub
  }
type typ_for_c =
  { c_func :  'self 's .
                a_trf -> b_trf -> c_trf -> d_trf -> fself:(c -> 's) ->
      'self show_c_stub
  }
type typ_for_d =
  { d_func :  'self 's .
                a_trf -> b_trf -> c_trf -> d_trf -> fself:(d -> 's) ->
      'self show_d_stub
  }
