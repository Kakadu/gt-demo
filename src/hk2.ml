(* Tying to make one big FIX: attempt 2  *)
open Utils
open Printf
...
(* We will have an issue in fix definition that type is less general then another.
   The reason is because synthesized for type paramter is in a_trf with a quantifier
   and below it is without
 *)
(* type ('inh, 'sa, 'sb, 'saparam) typ_for_a =
 *   { a_func : 'selfa 'p  .
 *                'sa a_trf -> 'sb b_trf -> fself:('p a -> 'sa) ->
 *                ('p -> 'saparam) ->
 *       ('inh, 'p, 'saparam, 'inh, 'selfa, 'sa) a_t
 *   }
 * type ('inh, 'sa, 'sb, 'saparam) typ_for_b =
 *   { b_func : 'self  .
 *                 'sa a_trf -> 'sb b_trf -> fself:(b -> 'sb) ->
 *       (('inh, 'self, 'sb) b_t)
 *   }
 *
 * let (a0,b0) =
...
type ('inh, 'sa, 'sb) typ_for_a =
  { a_func : 'selfa 'p 'saparam .
               'sa a_trf -> 'sb b_trf -> fself:('p a -> 'sa) ->
               ('p -> 'saparam) ->
      ('inh, 'p, 'saparam, 'inh, 'selfa, 'sa) a_t
  }
type ('inh, 'sa, 'sb) typ_for_b =
  { b_func : 'self 'saparam .
                'sa a_trf -> 'sb b_trf -> fself:(b -> 'sb) ->
      (('inh, 'self, 'sb) b_t)
  }
