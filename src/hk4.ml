...
type ('r, 'sr, 'sa1, 'sa2, 'sa, 'sb, 'r2) t2_a =
  { a_func : 'inh .
...
type ('r, 'sr, 'sa1, 'sa2, 'sa, 'sb, 'r2) t2_b =
  { b_func : 'inh .
      ('sa, 'sa1, 'r) a_trf -> ('sb, 'sa2, 'r2) b_trf ->
      ('r -> 'sr) ->
      'r b ->
...
let b0 =
  {b_func = (fun tra trb fa subj ->
          (gcata_b (new show_b_stub tra trb ~fself:(fun _ -> assert false) fa)
             ()
             subj)
      ) }
...
let myfix (a0,b0) =
  let rec show_a = { a_trf = fun fl subj ->
      a0.a_func show_a show_b fl subj
    }
  and     show_b = { b_trf = fun fl subj ->
      b0.b_func show_a show_b fl subj }
  in
  (show_a, show_b)
